...
MemorySystem::MemorySystem(const std::string &config_file,
    ...
    if (config_->IsHMC()) {
        ...
        std::cout << "[DRAMSim3] Starting a JedecDRAMSystem" << std::endl;
...
Config* MemorySystem::GetConfig() const { return config_; }
...
void MemorySystem::PrintStats(bool stdout) const { dram_system_->PrintStats(stdout); }

void MemorySystem::PrintDeadlock() const { dram_system_->PrintDeadlock(); }
