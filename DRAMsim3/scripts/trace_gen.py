...
class Generator():
    ...
    def __init__(self, stream_type, interarrival, ratio, gb, banks):
    ...
    def _custom_gen(self):
        op = 'r'
        sb_offset = 512 * 1024
        diff_b_offset = 128 * 1024
        #diff_b_offset = 16 * 1024
        num_banks = self._banks

        idx = (self._last_clk // self._interval) % (num_banks + 10)
        bank = idx % num_banks

        addr = (self._last_rd_addr + diff_b_offset * bank + sb_offset * idx)
        return (op, addr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--stream-type', default='random',
                        help='Address stream type, (r)andom, (s)tream, (m)ix, (c)ustom (default: random')
    parser.add_argument('-b', '--banks', type=int, default=8,
    ...
    files = {}
    for f in formats:
        file_name = '{}_{}_i{}_n{}_rw{}_b{}.trace'.format(
            f, stream_type, args.interarrival, args.num_reqs, int(args.ratio), args.banks)
        if f == 'dramsim2':
    ...
    g = Generator(stream_type, args.interarrival, args.ratio, args.gb, args.banks)
